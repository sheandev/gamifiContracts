    ...
    ReentrancyGuard
{
    ...
    event SetMaxTokenCounter(uint256 indexed maxTokenCounter);
    ...
    function initialize(
        ...
        uint256 maxTokenCounter
    ) public initializer {
        _setOwner(owner_);
        _name = name_;
        _symbol = symbol_;
        _maxTokenCounter = maxTokenCounter;
    }
    ...
    // custom functions
    function setFreezeStatus(uint256 tokenId, bool frozen)
        public
        virtual
        onlyAdmin
    {
    ...
    function setStartTime(uint256 tokenId, uint256 time)
        public
        virtual
        onlyAdmin
    {
    ...
    function setCounter(uint256 tokenId, uint256 counter)
        public
        virtual
        onlyAdmin
    {
    ...
    function setMaxTokenCounter(uint256 maxTokenCounter) public virtual onlyAdmin {
        require(maxTokenCounter > 0, "Invalid maxTokenCounter");
        _maxTokenCounter = maxTokenCounter;
        emit SetMaxTokenCounter(_maxTokenCounter);
    }
    ...
    function mintMemberCard(
        ...
        require(_tokenCounter < _maxTokenCounter, "Sold out");
        uint256 tokenId = _tokenCounter;
        _mint(user, tokenId);
        _setTokenURI(tokenId, tokenURI_);
        _membership[tokenId] = Membership(startTime, counter, typeId);
        _tokenCounter++;
    }

    function mintMemberCard(address user, uint256 typeId, string memory tokenURI_)
        public
        virtual
        onlyAdmin
    {
        ...
        _mint(user, tokenId);
        _setTokenURI(tokenId, tokenURI_);
        _membership[tokenId] = Membership(block.timestamp, 3, typeId);
        _tokenCounter++;
    }

    function getMemberCardActive(uint256 tokenId) public view returns (bool) {
        bool active = (_membership[tokenId].counter > 0 && !_frozen[tokenId]);
        return active;
    }

    function getMemberCard(uint256 tokenId)
        public
        view
    ...
    function getMaxTokenCounter() public view returns (uint256) {
        return _maxTokenCounter;
    }

    function isMember(address user) public view returns (bool) {
        uint256 balance = MemberCardV2.balanceOf(user);
        bool member;
        uint256 tid;
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner_ = MemberCardV2.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            MemberCardV2.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(MemberCardV2.ownerOf(tokenId), to, tokenId);
    }
    ...
    {
        require(
            index < MemberCardV2.balanceOf(owner_),
    ...
    function tokenByIndex(uint256 index) public view virtual returns (uint256) {
        require(
            index < MemberCardV2.totalSupply(),
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = MemberCardV2.balanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId)
        private
    {
        uint256 lastTokenIndex = MemberCardV2.balanceOf(from) - 1;
    ...
    function _beforeTokenTransfer(
        ...
        require(
            (getMemberCard(tokenId).counter == 0 || from == address(0)),
